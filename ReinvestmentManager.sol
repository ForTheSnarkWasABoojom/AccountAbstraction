...
contract ReinvestmentManager  {
    ...
    function addUserBatch(uint256[] memory userIDs_, string[] memory salt_, uint256[] memory balance_) external  onlyAdminOrEP{
    ...
    function reinvestSavings() external onlyAdminOrEP {
    ...
    function setRate(uint256 rate_) external onlyAdminOrEP {
        _rate = rate_;
        reinvestmentPeriod.rate =rate_;
    }

    function setAssetPrice(uint256 price_) external onlyAdminOrEP {
        _asset_price = price_;
    ...
    modifier onlyAdminOrEP() {
        _checkAllowed();
        _;
    }

    function _checkAllowed() internal view virtual {
        require(msg.sender == admin || msg.sender == entryPoint, "no permission");
    }
    ...
    function setEntryPoint(address _entryPoint) external onlyAdminOrEP {
